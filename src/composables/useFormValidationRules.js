...
export const minLength = (len) => (val) =>
	String(val).length >= len || `Min ${len} chars`;
export const passwordMatch = (otherPasswordRef) => (val) =>
	val === otherPasswordRef.value || 'Passwords must match';
...
export const coordsRules = [
	(v) =>
		!v ||
		coordsDec(v) ||
		coordsDMS(v) ||
		'Enter valid coordinates in decimal (e.g. 48.8584) or DMS format (e.g. 48°51\'29.6"N)',
];
