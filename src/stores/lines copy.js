...
export const useLinesStore = defineStore('lines', () => {
	...
	// actions
	function setTripId(newTripId) {
		tripId.value = newTripId;
	}

	// Watch for changes to tripId
	watch(tripId, (newTripId, oldTripId) => {
		if (newTripId) {
			watchEffect(() => console.log(`linesStore, watch: tripId changed from ${oldTripId} to ${newTripId}`));
			loadLines(newTripId); // Trigger loadLines when tripId changes
		}
	});
